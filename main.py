...
parser.add_argument("-x", "--number", type=int, default=0)
...
if __name__ == '__main__':
    ...
    methods = ['Random', 'UncertainGCN', 'CoreGCN', 'CoreSet', 'lloss', 'VAAL', 'daeho', 'cam', 'CAAL', 'ensemble']
    ...
    for trial in range(TRIALS):
        ...
        for cycle in range(CYCLES):
            ...
            if method == 'ensemble':
                ...
                acc, confusion_matrix = test(models, EPOCH, method, dataloaders, mode='test')

                with open('confusion_matrix.txt', 'a') as f:
                    f.write(str(cycle + 1) + ' : ' + str(confusion_matrix) + "\n")
            ...
            elif not method == 'daeho' and not method == 'jundoo':
                train(models, method, criterion, optimizers, schedulers, dataloaders, args.no_of_epochs, EPOCHL)
                ...
                if method == "CAAL":
                    arg, labeled_graycam, unlabeled_graycam = query_samples(models, method, data_unlabeled, subset,
                                                                            labeled_set, cycle, args)
                else:
                    arg = query_samples(models, method, data_unlabeled, subset, labeled_set, cycle, args)
            ...
            if not spl:
                ...
                listd = list(
                    torch.cat([torch.tensor(subset)[arg][:v], torch.tensor(subset)[arg][v + ADDENDUM:]]).numpy())
                unlabeled_set = listd + unlabeled_set[SUBSET:]
                print(len(labeled_set), min(labeled_set), max(labeled_set))
                # Create a new dataloader for the updated labeled dataset
                dataloaders['train'] = DataLoader(data_train, batch_size=BATCH,
