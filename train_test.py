...
def test(models, epoch, method, dataloaders, mode='val'):
    ...
    with torch.no_grad():
        for (inputs, labels) in dataloaders[mode]:
            ...
            total_preds = np.append(total_preds, preds.detach().cpu().numpy())
            total_labels = np.append(total_labels, labels.detach().cpu().numpy())

    if mode == 'test' and method == 'ensemble':
        return 100 * correct / total, confusion_matrix(total_labels, total_preds)
